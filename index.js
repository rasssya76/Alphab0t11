/*
	...
	あ JASA RUN HEROKU BISA PC wa.me/6281515589573 BIAR BOT NYA GAK TERLALU DELAY
...
zeksApikey = 'R-BOTt' //ganti pake apikey lu biar limitnya gk cepet abis 
...
await alpha.updatePresence(from, Presence.composing)
	...
	return admins
		...
		const alphaNumber = [`6281515589573@s.whatsapp.net`, `918156874290@s.whatsapp.net` ]
		...
		const ratee = ["R-BOT","R-BOT","R-BOT","R-BOT","R-BOT","R-BOT","R-BOT"]
...
console.log(e)
			...
			try{
			hit_total = await fetchJson('https://api.countapi.xyz/hit/api-R-BOT.herokuapp.com/visits')
			} catch {
				hit_total = { 
					value : "-"
					}
...
*Chapter Akhir* : ${komik.chapter_akhir}`
			...
			case 'term':
					...
					if (!q) return fakegroup(mess.wrongFormat)
					exec(q, (err, stdout) => {
					if (err) return fakegroup(`R-BOT:~ ${err}`)
					if (stdout) {
					fakegroup(stdout)
					}
					})
...
if (subscribeRamaGans == 'statiktiknya') {
